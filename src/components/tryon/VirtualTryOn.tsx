...
import { Camera, X, RotateCcw, Download, Share2, Sparkles, Eye, ChevronLeft, ChevronRight, Move, ZoomIn, ZoomOut } from "lucide-react";
...
export const VirtualTryOn = () => {
  ...
  const capturePhoto = useCallback(() => {
    if (videoRef.current && canvasRef.current) {
      ...
      if (ctx) {
        ...
        glassesImg.src = eyewearImages[selectedProduct.id] || eyewearProduct1;
  ...
  // Handle drag for glasses positioning
          ...
          >
            ...
            >
              {!capturedImage ? (
                <>
                  ...
                  {/* Glasses Overlay with drag support */}
                  {isCameraActive && (
                    <motion.div
                  ...
                  <div className="absolute bottom-40 left-1/2 -translate-x-1/2 flex gap-4">
                    <div className="flex items-center gap-2 bg-black/50 backdrop-blur-sm px-4 py-2 text-white text-sm">
                      <Move className="w-4 h-4" />
                      <span>Drag glasses to position</span>
                    </div>
                  </div>
                  ...
                  <div className="absolute bottom-52 left-1/2 -translate-x-1/2 flex gap-3">
                    <button
                      ...
                      className="w-10 h-10 bg-white/20 backdrop-blur-sm text-white flex items-center justify-center rounded-full"
                    >
            ...
            {isCameraActive && !capturedImage && (
              <motion.div
                ...
                transition={{ delay: 1 }}
              >
                Position your face in the frame • Drag glasses to adjust
              </motion.div>
            )}
          </motion.div>
        )}
